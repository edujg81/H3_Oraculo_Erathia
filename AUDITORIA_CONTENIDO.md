...
Fuentes internas prioritarias: `REGLAMENTO_DEFINITIVO.md`, `src/data/reglasCombinadas.ts`. Fuentes derivadas o auxiliares: `src/data/rulesKB.ts`, `REGLAS_Y_FAQS.md`, `MANUAL.md` y los reglamentos indexados en el proyecto. La fuente normativa final sigue siendo el reglamento castellano del juego de mesa.
...
- `[~] Pergaminos de Hechizo`: `reglasCombinadas.ts` los asocia a la expansión Bastión, mientras `REGLAS_Y_FAQS.md` los asocia a Conflujo. **EVIDENCIA PDF (HoMM-Conflux-Mission-Book_ESP.pdf):** los pergaminos **no aparecen como novedad de Conflujo**, solo se mencionan en la sección de reglas de campaña entre escenarios donde se indican que NO forman parte del mazo y no pueden conservarse entre escenarios. **EVIDENCIA PDF (HoMM-Stronghold-Mission-Book_ESP.pdf):** los pergaminos **sí se introducen como novedad de la expansión Bastión** (línea ~45: "Esta expansión introduce ... pergaminos de hechizos", línea ~70: lista de componentes con "Cartas de pergamino de hechizos", línea ~95-100: descripción de preparación y uso). **Conclusión:** La atribución correcta es a la expansión Bastión, no a Conflujo. Estado: PENDIENTE (reconciliar `reglasCombinadas.ts` con la fuente normativa).**
...
- `[~] Recursos iniciales por escenario específico`: presupuesto base confirmado (15 Oro/3 Materiales de construcción/1 Objeto de valor), pero **EVIDENCIA PDF** varía por escenario:
...
- [~] Confirmar la atribución de los Pergaminos de Hechizo a la expansión correcta (Bastión vs Conflujo). **EVIDENCIA PDF:** HoMM-Stronghold-Mission-Book_ESP.pdf confirma pergaminos como mecánica de Bastión (líneas ~45-100); HoMM-Conflux-Mission-Book_ESP.pdf NO los menciona como novedad. Estado: ACTUALIZADO (Bastión).
...
- [~] Verificar la expansión del Cañón (Cala) contra PDFs oficiales (`HoMM-Cove-Mission-Book_ESP.pdf`). **EVIDENCIA PDF:** `HoMM-Cove-Mission-Book_ESP.pdf` (líneas 1042 y 1240) confirma que el Cañón es una máquina de guerra especial de Cala. `rulesKB.ts` ya indica "(Cannon - Cala)". `reglasCombinadas.ts` solo listaba 4 máquinas (sin cañón) en la sección de Murallas, pero ahora se ha añadido. Estado: ACTUALIZADO (Cañón = Cala, corregido en reglasCombinadas.ts).
